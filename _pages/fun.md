  ...
  <div class="sudoku-accordion" data-sudoku-accordion>
    ...
    <div class="sudoku-panels">
      {% for puzzle in site.data.sudoku %}
        ...
        >
          <div class="title">
            {{ puzzle.title }} (<a href="https://sudokupad.app/{{ puzzle.sudokupad_id }}">SudokuPad</a>){% if puzzle.lmd_id %}
              (<a href="https://logic-masters.de/Raetselportal/Raetsel/zeigen.php?id={{ puzzle.lmd_id }}">LMD</a>){% endif %}{% if puzzle.ctc_id %}
              (<a href="https://www.youtube.com/watch?v={{ puzzle.ctc_id }}">CTC</a>){% endif %}
          </div>

          {% assign puzzle_image_filename = puzzle_id | replace: "-", "_" | append: ".png" %}
          {% assign puzzle_image_path = "assets/img/" | append: puzzle_image_filename %}
          {% assign image_static_path = puzzle_image_path | prepend: "/" %}
          ...
          {% if puzzle_image %}
            ...
            <div class="col-sm-8">
              <a
                href="https://sudokupad.app/{{ puzzle.sudokupad_id }}"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Open {{ puzzle.title }} in SudokuPad"
              >
                {%
                  include figure.liquid
                  path=puzzle_image_path
                  title=puzzle.title
                  alt=puzzle_alt
                  width=900
                  height=900
          ...
          {% else %}
            <!-- Image placeholder: add {{ puzzle_image_path }}. -->
          {% endif %}
